...
async function generateRustMod(schemaFiles: string[]) {
  const generatedDir = join(REPO_ROOT, 'packages/crypto/src/generated');
  const schemasDir = join(generatedDir, 'schemas');
  const modules = schemaFiles.map((f) => basename(dirname(f)));

  // Generate the parent mod.rs that declares the schemas submodule
  const parentMod = `//! Auto-generated code
//!
//! DO NOT EDIT - Files in this module are auto-generated by tools/codegen

pub mod schemas;
`;
  await writeFile(join(generatedDir, 'mod.rs'), parentMod);
  logv(`  ✅ Rust: generated/mod.rs`);

  // Generate the schemas/mod.rs that lists individual schema modules
...
`;
  ...
  await writeFile(join(schemasDir, 'mod.rs'), code);
  logv(`  ✅ Rust: generated/schemas/mod.rs`);
}
